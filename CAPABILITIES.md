# Capabilities — everything ClaudeBridge can do in DaVinci Resolve

This is the full action reference for `bridge/ClaudeBridge.lua` (Path B —
DaVinci Resolve **Free** or Studio, see the main [README](README.md) for the
two-path overview). Started as the read/write core of `server.py`'s tool set
and has since grown past it: Fusion node-graph building/inspection and a
dedicated set of banner/overlay-image placement actions (useful any time you
need to drop a sequence of still images onto a timeline at specific frames —
lower-thirds, title cards, chapter banners, etc.) are now included too.
Render jobs and color/LUT actions are still not ported — see
[Known limitations](BRIDGE_SETUP_GUIDE.md#known-limitations).

For install steps and how the file-based command queue actually works, see
[BRIDGE_SETUP_GUIDE.md](BRIDGE_SETUP_GUIDE.md). This file is just the
capability list.

Each command in the queue looks like:
...
Not ported to ClaudeBridge yet (still Path A / `server.py` only): render
preset & job management, and LUT application. See
[Known limitations](BRIDGE_SETUP_GUIDE.md#known-limitations) for the current
gaps and contribution notes.
